...
import {
	valueNoise3dFactory as noiseFactory,
	randomFactory,
} from '../../src/squeaker';
import { getRandomLargePrime } from '../../src/util';

const noiseData: number[][][] = [];
const generalNoiseGenerator = noiseFactory();
let min = 1,
	max = 0;
...
test('Basic 3d test', (t) => {
	t.is(typeof generalNoiseGenerator(0, 0, 0), 'number');
});
...
test('value 3d is repeatable/isomorphic', (t) => {
...
test('respects ranges', (t) => {
	...
	const noiseNoBounds = noiseFactory({
		random: random.random,
	});
